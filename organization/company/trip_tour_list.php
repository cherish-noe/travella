...
<html>
<head>
<title>TRAVELLA:Trip and Tour List</title>
...
<body class="cbp-spmenu-push">
	<div class="main-content">
        ...
        <div id="page-wrapper">
            ...
            <div class="lists">
			<h3>Trip Lists:</h3>
<form action="choose_bus.php" method="post">
<?php 
$companyID=$_SESSION['USERID'];
$viewTrip="SELECT t.* 
		FROM trip t,user u 
		WHERE u.USERID=t.USERID 
		AND u.USERID='$companyID'
		AND TRIP_TYPE='Trip'";
$ret=mysql_query($viewTrip);
$count=mysql_num_rows($ret);

if ($count==0) 
{
	echo " <div class='alert pending'>
	<span class='closebtn'>&times;</span>No Record for Trip!</div>";
}
else
{
	...
	{ 
		...
		$tripID=$data['TRIPID'];
		echo "<input type='hidden' name='txtTripID' value='$tripID'";
		echo "<tr>";
		echo "<td>".$data['ORIGIN']."</td>";
		echo "<td>".$data['DESTINATION']."</td>";
		echo "<td>".$data['DEPARTURE_DATE']."|".$data['DEPARTURE_TIME']."</td>";
		echo "<td>".$data['VENUE']."</td>";
		echo "<td>".$data['DURATION']."</td>";
		echo "<td>".$data['PRICE']."</td>";
		echo "<td><button name='btnchoose' class='j_choose'>Choose Bus</button></td>";
		echo "</tr>";
	}
	echo "</table>";
}
?>
<h3>Tour Package Lists:</h3>
<form action="choose_bus.php" method="post">
<?php 
$companyID=$_SESSION['USERID'];
$viewTour="SELECT t.* 
		...
		AND u.USERID='$companyID'
		AND TRIP_TYPE='Tour'";
$rets=mysql_query($viewTour);
$counts=mysql_num_rows($rets);

if ($counts==0) 
{
	echo " <div class='alert pending'>
	<span class='closebtn'>&times;</span>No Record for Tour Package!</div>";
}
else
{
		echo "<table align='center'>";
	...
	for ($i=0; $i < $counts; $i++) 
	{ 
		$datas=mysql_fetch_array($rets);
		$tripID=$datas['TRIPID'];
		echo "<input type='hidden' name='txtTripID' value='$tripID'";
		echo "<tr>";
		echo "<td>".$datas['ORIGIN']."</td>";
		echo "<td>".$datas['DESTINATION']."</td>";
		echo "<td>".$datas['DEPARTURE_DATE']."|".$datas['DEPARTURE_TIME']."</td>";
		echo "<td>".$datas['VENUE']."</td>";
		echo "<td>".$datas['DURATION']."</td>";
